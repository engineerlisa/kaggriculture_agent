...
TASK_PRIORITY = {"critical_feed": 0, "critical_water": 0,
                 "pickup_wheat": 1, "harvest_animal": 1, "harvest": 1,
                 "feed": 2, "water": 2,
                 "care": 3, "collect_fertilizer": 3,
                 "pickup_animal": 3, "place_animal": 3, "build_structure": 3,
                 "plant": 4, "weed": 5,}
...
def _growing_crop_supply(farm, crop, day):
    ...
    for row in farm["tiles"]:
        for tile in row:
            ...
            if not crop_data["ongoing"]:
                total_supply += UNFERTILIZED_YIELDS[crop]
                continue
...
def _committed_crop_supply(ctx, crop):
    ...
    my_pipeline = _growing_crop_supply(ctx.me, crop, ctx.day)

    opponent_pipeline = _growing_crop_supply(ctx.opponent, crop, ctx.day)
...
def _animal_product_supply(farm, product, current_day):
    held_units = 0
    future_events = {}

    for _, _, tile in _animal_tiles(farm):
        animal_data = ANIMALS[tile["animal"]]
...
def _projected_animal_revenue(ctx, animal):
    ...
    my_held, my_events = _animal_product_supply(ctx.me, product, ctx.day)
    opponent_held, opponent_events = _animal_product_supply(ctx.opponent, product, ctx.day)
...
def _task_is_desirable(ctx, task):
    ...
    if task.type in ("plant", "build_structure"):
        return ctx.hour < 23
...
def _market_actions(ctx, crop_to_plant, animal_to_add):
    actions = []
    ...
    for item_in_shed, inventory in ctx.private["shed"].items():
...
def _sell_quantity(ctx, crop, available):
    ...
    min_price = MARKET_PARAMS[crop]["base"] * 0.8
...
def agent(obs):
    ...
    return {
        ...
        "market": _market_actions(ctx, crop_to_plant, animal_to_add),
    }