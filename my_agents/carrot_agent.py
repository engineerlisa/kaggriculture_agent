...
def _find_tasks(ctx):
    ...
    have_seed = _have_seed(ctx, "CARROT")
    ...
    for y, row in enumerate(ctx.me["tiles"]):
        for x, tile in enumerate(row):
            ...
            if tile is None:
                if have_seed:
                    tasks.append(Task(type="plant", x=x,  y=y, crop="CARROT", tile=tile))
                continue
...
def _market_actions(ctx):
    ...
    carrot_seeds = ctx.private["seeds"].get("CARROT", 0)
    carrot_in_shed = ctx.private["shed"].get("CARROT", 0)
    ...
    if carrot_seeds == 0 and ctx.me["money"] >= 60:
        actions.append(["BUY_SEED", "CARROT", 3])

    if carrot_in_shed > 0 and ctx.market["prices"]["CARROT"] > 33:
